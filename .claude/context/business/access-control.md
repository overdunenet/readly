...
| 접근 레벨 | 가격 규칙 | 보정 위치 |
| --------- | --------- | --------- |
| `public` | 항상 0원 (서버 강제) | PostEntity.edit() |
| `subscriber` | 에디터 설정값 유지 | - |
| `purchaser` | 에디터 설정값 유지 | - |
| `private` | 에디터 설정값 유지 | - |

> 전체공개(public) 포스트는 가격을 설정할 수 없습니다. 클라이언트에서 어떤 값을 보내더라도 서버에서 0으로 강제 보정합니다.
...
| ---- | --- | ---- |
| 전체 | `all` | 모든 연령 열람 가능 |
| 성인 | `adult` | 성인 인증 후 열람 가능 |

- 발행 시 에디터가 연령 등급을 선택합니다
- 재발행 시 기존 연령 등급이 유지됩니다
