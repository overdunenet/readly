...
이 Skill은 `gpm` CLI를 통해 GitHub Project V2의 태스크를 관리합니다.
프로젝트 루트에 `.gpmrc` 파일이 있어야 동작합니다. 없으면 `gpm init`을 먼저 실행하세요.
...
6. 사용자가 선택한 태스크를 "In Progress"로 변경합니다:

   ```bash
   gpm task status <id> "In Progress"
   ```

7. 선택한 태스크에 대해 구현 계획을 수립합니다:
...
<constraints>
- .gpmrc 파일이 없으면 "gpm init을 먼저 실행하세요"라고 안내
