...
| 유형               | 설명                  | 워크플로우                             |
| ------------------ | --------------------- | -------------------------------------- |
| **Feature**        | 새로운 기능 기획      | 전체 PRD Phase A→E                     |
| **Refactor/Infra** | 리팩토링, 인프라 변경 | PRD 없이 → task-planner의 plan.md 직행 |
| **Bugfix**         | 버그 수정             | PRD 없이 → GitHub Issue만 생성         |

> Feature가 아닌 작업은 이 워크플로우를 거치지 않는다. pm-planner의 `/pm > New Task > Issue`로 처리한다.
...
PRD가 approved/decomposed 상태가 되면, 구현 단계는 PM 워크플로우를 벗어난다.

### 핸드오프 규칙

1. **PM의 역할은 PRD까지**: PRD(What/Why)를 작성하고 승인받는 것이 PM의 책임 범위
2. **구현 분석은 task-planner의 영역**: PRD를 읽고 코드베이스를 분석하여 plan.md(How)를 작성하는 것은 task-planner Agent의 역할
3. **Implementation Guide 미생성**: PRD 산출물에 Implementation Guide를 포함하지 않는다. 구현 방법은 task-planner가 코드베이스를 직접 분석하여 결정한다

### plan.md가 참조하는 PRD 섹션 매핑

| PRD 섹션                 | plan.md 활용                       |
| ------------------------ | ---------------------------------- |
| §1 개요 + 핵심 설계 결정 | plan.md 목적(Why) + 설계 결정 요약 |
| §3 유저스토리            | TaskList 분해 기준                 |
| §7 수용 기준 (AC)        | 완료 조건 + 테스트 시나리오        |
| §8 기술 요건             | DB/API/FE 설계의 출발점            |
| §10 Non-Goals            | 구현 범위 제한                     |
