...
| [post-editor.md](./post-editor.md) | BlockNote 포스트 에디터 UI 및 S3 이미지 업로드 |
| [post-manage-ui.md](./post-manage-ui.md) | 에디터 포스트 목록 관리 UI (필터, 검색, 액션) |
| [upload-module.md](./upload-module.md) | S3 Presigned URL 기반 이미지 업로드 모듈 (API + Client) |
...
- **파일 저장소**: AWS S3 + CloudFront CDN (Presigned URL 업로드)

### Entity 요약

**UserEntity**: email, nickname, profileImage, phone(nullable), socialAccounts
**SocialAccountEntity**: userId, provider, accountId (ManyToOne with User)
**PostEntity**: freeContent, paidContent, accessLevel, status, price, bookstoreId(nullable)
**BookstoreEntity**: userId(unique), penName, storeName, bio, publishDefault(embedded), OneToOne with User
**PublishDefaultEntity**: defaultAccessLevel, defaultPrice, defaultAgeRating (Embedded in BookstoreEntity)
...
- `business/image-upload.md`: 이미지 업로드 정책
